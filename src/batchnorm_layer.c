...
layer make_batchnorm_layer(int batch, int w, int h, int c) {
  ...
  l.expand_a = calloc(c, sizeof(float));
  l.expand_b = calloc(c, sizeof(float));
...
void expand_rolling_mean_variance(layer *l) {
  int i;
  for (i = 0; i < l->out_c; i++) {
    float sqrt_var = sqrt(l->rolling_variance[i]) + .000001f;
    l->expand_a[i] =
        l->biases[i] - l->scales[i] * l->rolling_mean[i] / sqrt_var;
    l->expand_b[i] = l->scales[i] / sqrt_var;
  }
}
...
void normalize_cpu_thread(struct normalize_params *params, size_t batch,
                          size_t filters) {
  float *ptr =
...
void forward_batchnorm_layer(layer l, network net) {
  TIME_BEGIN(forward_batchnorm_layer);
  if (l.type == BATCHNORM)
  ...
  struct normalize_params params = {l.output, l.expand_a, l.expand_b,
                                    l.out_h * l.out_w};
  pthreadpool_compute_2d(net.threadpool,
                         (pthreadpool_function_2d_t)normalize_cpu_thread,
  ...
  TIME_END(forward_batchnorm_layer);
}
