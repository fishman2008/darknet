  ...
  *(l.cost) = 0;
  for (b = 0; b < l.batch; ++b) {
      ...
      for (t = 0; t < 30; ++t) {
        ...
        if (truth.x > 100000 && truth.y > 100000) {
          ...
          delta_region_class(l.output, l.delta, class_index, class, l.classes,
                             l.softmax_tree, l.class_scale, l.w * l.h,
                             &avg_cat);
    ...
    for (t = 0; t < 30; ++t) {
      ...
      // printf("%d %f (%f, %f) %f x %f\n", best_n, best_iou, truth.x, truth.y,
      // truth.w, truth.h);
...
void get_region_boxes(layer l, int w, int h, int netw, int neth, float thresh,
                      ...
                      int only_objectness, int *map, float tree_thresh,
                      int relative) {
  int i, j, n, z;
  ...
  for (i = 0; i < l.w * l.h; ++i) {
    ...
    for (n = 0; n < l.n; ++n) {
      ...
      boxes[index] = get_region_box(predictions, l.biases, n, box_index, col,
                                    row, l.w, l.h, l.w * l.h);
      if (masks) {
      ...
      if (l.softmax_tree) {
        ...
        for (j = 0; j < l.classes; ++j) {
          ...
          /*
             ...
             int blacklist[] = {121, 497, 482, 504, 122, 518,481, 418, 542, 491,
             914, 478, 120, 510,500};
             int bb;
...
void forward_region_layer_gpu(const layer l, network net) {
  ...
  if (l.softmax_tree) {
    ...
    softmax_gpu(net.input_gpu + index, group_size, l.batch*l.n, l.inputs/l.n,
    ...
    for (i = 0; i < l.softmax_tree->groups; ++i) {
    int group_size = l.softmax_tree->group_size[i];
    softmax_cpu(net.input + count, group_size, l.batch, l.inputs, l.n*l.w*l.h,
    1, l.n*l.w*l.h, l.temperature, l.output + count);
    count += group_size;
    }
    }
    cudaDeviceSynchronize();
