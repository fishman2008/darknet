...
static void print_cocos(FILE *fp, char *image_path, box *boxes, float **probs,
  ...
  for (i = 0; i < num_boxes; ++i) {
    ...
    for (j = 0; j < classes; ++j) {
      if (probs[i][j])
        fprintf(fp, "{\"image_id\":%d, \"category_id\":%d, \"bbox\":[%f, %f, "
                    "%f, %f], \"score\":%f},\n",
...
void validate_detector_flip(char *datacfg, char *cfgfile, char *weightfile,
  ...
  for (i = nthreads; i < m + nthreads; i += nthreads) {
    ...
    for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
      ...
      int w = val[t].w;
      int h = val[t].h;
      get_region_boxes(l, w, h, net.w, net.h, thresh, probs, boxes, 0, 0, map,
                       .5, 0);
      if (nms)
      ...
      if (coco) {
        ...
        print_imagenet_detections(fp, i + t - nthreads + 1, boxes, probs,
                                  l.w * l.h * l.n, classes, w, h);
      } else {
        print_detector_detections(fps, id, boxes, probs, l.w * l.h * l.n,
                                  classes, w, h);
      }
      free(id);
      free_image(val[t]);
...
void validate_detector_recall(char *cfgfile, char *weightfile) {
  ...
  for (i = 0; i < m; ++i) {
    ...
    get_region_boxes(l, sized.w, sized.h, net.w, net.h, thresh, probs, boxes, 0,
                     1, 0, .5, 1);
    if (nms)
